                  ...
                  children: [
                    Container(
                      ...
                      margin: EdgeInsets.only(
                          left: responsive.w(16), bottom: responsive.h(2)),
                      child: Material(
                              ...
                              child: TextField(
                                ...
                                decoration: InputDecoration(
                                  ...
                                  hintText: communityPost!.id ==
                                      ...
                                      ? "Add a comment"
                                      : "Replying to: ${currentlyCommentingPost!.content!.length > 23 ? currentlyCommentingPost!.content!.substring(0, 20) + "..." : currentlyCommentingPost!.content!}",
                                  hintStyle: TextStyle(
                                    color:
...
class _CommentState extends State<Comment> {
  ...
  Widget build(BuildContext context) {
    ...
    if (comment != null) {
      if (comment!.postedMinutes != null) {
        if (comment!.postedMinutes! > 1440) {
          timeToShow = DateFormat("dd MMM, yyyy")
              .format(DateTime.parse(comment!.timeOfCreation!));
    ...
    return comment != null
              ...
              children: [
                Column(
                  children: [
                    ...
                    Container(
                      height: 1,
                      width: (widget.comment.threadRank! > 2)
                          ? responsive.w(10)
                          : 0,
                      color: Color.fromRGBO(220, 220, 220, 1),
                    )
        ...
        //   //   radius: 16,
        //   // ),
        //   // Check if the URL is valid (not null and not empty)

        //   leading: CircleAvatar(
        //     // Use a CircleAvatar for consistent shape
        //     radius: 18,
        //     backgroundColor: Colors.transparent,  // Adjust size as needed
        //     child: comment!.postedBy?.userProfilePictureUrl != null && comment!.postedBy!.userProfilePictureUrl!.isNotEmpty
  ...
  List<Widget> _buildCommentList(ThemeData theme, CommunityPost communityPost) {
    ...
    if (showingComments) {
      if (loading) {
        return [
          Container(
            ...
            child: Container(
              padding: EdgeInsets.symmetric(
                  horizontal: responsive.w(5), vertical: responsive.h(5)),
      ...
      int n = communityPost.comments?.length ?? 0;
      return (communityPost.comments?.asMap().entries.map((entry) {
                int index = entry.key;
                CommunityPost c = entry.value;
                bool isLast = index == n - 1;
