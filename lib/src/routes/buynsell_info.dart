...
class BuyAndSellInfoPage extends StatefulWidget {
  final Future<BuynSellPost?> postFuture;
...
class _BuyAndSellInfoPageState extends State<BuyAndSellInfoPage> {
      ...
      body: Center(
        child: Column(
          ...
          children: [
            ...
            SizedBox(height: 24),
            ElevatedButton(
              onPressed: () => setState(() => _postFuture = widget.postFuture),
              child: Text('Retry'),
            ),
          ],
        ),
  ...
  Widget _buildContent(BuynSellPost post) {
    ...
    final phoneNumber = post.contactDetails ?? '';
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        // actions: [_buildBookmarkButton(post)],
      ),
      body: SingleChildScrollView(
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            _buildImageGallery(post, imageHeight, screenWidth, phoneNumber),
            _buildProductInfo(post, screenWidth),
          ],
        ),
      ),
      bottomNavigationBar: _buildBottomActionBar(phoneNumber),
    );
  }
  ...
  Widget _buildImageGallery(BuynSellPost post, double imageHeight, double screenWidth, String phoneNumber) {
    ...
    return Container(
      ...
      child: Stack(
        children: [
          if (images.isNotEmpty) _buildImagePager(images, imageHeight, phoneNumber),
  ...
  Widget _buildImagePager(List<String> images, double imageHeight, String phoneNumber) {
    return GestureDetector(
      onTap: () {
        Navigator.push(
          context,
  ...
  Widget _buildSellerInfo(String userName, String rollNumber, String? timeBefore) {
    return Row(
      mainAxisAlignment: MainAxisAlignment.spaceBetween,
      children: [
        Row(
  ...
  String _buildFullDescription(BuynSellPost post) {
    ...
    final brand = post.brand != null ? 'Brand: ${post.brand}\n' : '';
    final condition = post.condition != null ? 'Condition: ${post.condition}/10\n' : '';
    final warranty = post.warranty != null ? 'Warranty: ${post.warranty! ? "Yes" : "No"}\n' : '';
    final packaging = post.packaging != null ? 'Packaging: ${post.packaging! ? "Yes" : "No"}\n' : '';
